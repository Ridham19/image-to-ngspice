...
\section{Project Pipeline: Hand-Drawn Circuit to Validated SPICE Netlist}
The digitization of hand-drawn circuits is a complex Electronic Design Automation (EDA) task that transforms unstructured physical data (pixels) into a structured mathematical model (a SPICE netlist). This process is critical for modernizing rapid prototyping and educational workflows. The pipeline is designed around a hybrid logic approach, combining state-of-the-art Deep Learning with deterministic Computer Vision algorithms.

This pipeline can be summarized in three main logical phases:

\subsection{Phase 1: Component Localization and Classification (The "Eyes")}
The input is a raw photograph of a hand-drawn circuit sketch. The challenge is to identify and differentiate between dozens of components drawn with varying styles, orientations, and scales. This system employs the \textbf{YOLO (You Only Look Once)} deep learning architecture, specifically fine-tuned for electronic symbols. As an end-to-end detector, YOLOv8 performs classification and localization simultaneously, outputting a set of bounding boxes with associated component labels (e.g., "Resistor", "Capacitor", "Voltage Source"). This is a critical domain-specific application of transfer learning in image processing.

\subsection{Phase 2: Topological and Parameter-Value Extraction (The "Logic")}
Once the components are identified, the system must understand how they are connected.
\begin{itemize}
    \item \textbf{Wire Tracing and Interconnects:} This deterministic phase utilizes the \textbf{OpenCV} library. Image preprocessing techniques like adaptive thresholding and morphological operations (dilation and closing) are used to clean the image and repair any discontinuities in the sketched wire traces. Specialized line-tracing algorithms then convert these enhanced pixel paths into mathematical representations of nodes and connections.
    \item \textbf{Parameter Extraction (Optical Character Recognition - OCR):} To read numerical values like "10k$\Omega$" or "5V" off the page, an OCR engine like EasyOCR is deployed. It transcribes handwritten text within or near a component's bounding box and applies a spatial-association algorithm to map the recognized value to the correct electronic part in the system's memory.
\end{itemize}

\subsection{Phase 3: Interactive Verification and Netlist Generation (The "Output")}
The raw machine learning outputs (component lists, bounding boxes, wire paths) are unified within the interactive PySpice Studio Graphical User Interface (GUI). This digital canvas allows the user to click, drag, and modify the AI's predictions—such as correcting a misclassified source type or manually editing a default component value—providing a necessary human-in-the-loop validation layer.

Once validated, the backend executes the mathematical netlist generation algorithm. This script algorithmically maps all connected wire segments into shared mathematical nodes (e.g., Ground node "0", Node "1", Node "OUT"). Finally, it iterates through the validated component list, formatting their specific SPICE syntax (e.g., \texttt{[Component\_Name] [Node\_1] [Node\_2] [Value]}) to synthesize the final executable \texttt{.cir} netlist file for NGSPICE simulation.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/pipeline.png}
    \caption{Project Pipeline Flowchart illustrating the journey from a raw hand-drawn circuit sketch to a validated NGSPICE netlist.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/IoU.png}
    \caption{Visual representation of Intersection over Union (IoU).}
    \label{fig:iou}
\end{figure}
...
\end{equation}
By minimizing $\mathcal{L}_{total}$ over hundreds of epochs, the neural network converges, learning the precise visual signatures of the 9 distinct electronic classes.
