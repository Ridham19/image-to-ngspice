...
By epoch 80, the loss curves began to plateau, indicating convergence. The model successfully learned the visual signatures of the 9 classes without overfitting to the training data.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/BoxPR_curve.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/BoxF1_curve.png}
    \caption{F1-Confidence curve demonstrating the harmonic mean of precision and recall across various confidence thresholds. The peak F1 score of 0.80 occurs at a confidence of 0.343.}
    \label{fig:f1_curve}
\end{figure}

The individual Precision-Confidence and Recall-Confidence curves provide further insight into the model's behavior. As shown in Figure~\ref{fig:p_curve}, demanding a higher confidence threshold increases the precision, ensuring that the detected bounding boxes are almost certainly correct. However, Figure~\ref{fig:r_curve} demonstrates the inherent trade-off: a higher threshold causes the recall to drop, meaning the model might miss some faint or ambiguous hand-drawn components. Balancing these two metrics is essential for the reliability of the PySpice Studio backend.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/BoxP_curve.png}
    \caption{Precision-Confidence curve showing how the accuracy of positive predictions increases as the confidence threshold is raised.}
    \label{fig:p_curve}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/BoxR_curve.png}
    \caption{Recall-Confidence curve illustrating the trade-off as the confidence threshold increases. At higher thresholds, the model's ability to find all ground-truth objects (recall) naturally decreases.}
    \label{fig:r_curve}
\end{figure}
...
\begin{figure}[H]
    \centering
     \includegraphics[width=0.95\textwidth]{C171_D1_P1/canvas_confirmation.png}
...
\vspace{0.5cm}
\textbf{Generated NGSPICE Netlist:}
\begin{verbatim}
* PySpice Studio Netlist
